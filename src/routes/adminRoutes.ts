...
router.post('/users', async (req: AuthRequest, res: Response): Promise<void> => {
  ...
  try {
    ...
    if (err.code === 'P2002') { res.status(409).json({ message: 'Email already exists' }); return; }
...
router.post('/branches', async (req: AuthRequest, res: Response): Promise<void> => {
  ...
  try {
    ...
    if (err.code === 'P2002') { res.status(409).json({ message: 'Email already in use' }); return; }
    res.status(500).json({ message: 'Server error' });
  }
});

router.put('/branches/:id', async (req: AuthRequest, res: Response): Promise<void> => {
  try {
...
router.get('/performance', async (_req, res: Response): Promise<void> => {
  try {
    const branches = await prisma.branch.findMany({
      include: {
